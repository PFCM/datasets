...
def online_block_tensors(batch_size, sequence_length, block_size,
    ...
    with tf.variable_scope('input'):
        ...
        if variable_sequence_length:
            ...
            sequence_lengths = tf.convert_to_tensor(sequence_length)

        start_positions = tf.random_uniform(
            [batch_size], minval=0, maxval=sequence_lengths-block_size,
            dtype=tf.int32)
        ...
        indices = tf.pack([
            tf.range(start, limit=start+block_size)
            for start in tf.unpack(start_positions)])
        mask = tf.sparse_to_dense(indices, sequences.get_shape())

        return tf.select(mask, ones, sequences)
